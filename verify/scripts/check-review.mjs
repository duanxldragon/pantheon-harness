...
function validateMethodReview(review, errors) {
  ...
  const validLeakageStates = new Set(['none', 'accepted', 'blocked']);

  requireNonEmptyString(methodReview, 'ownerLayer', errors, 'root.methodReview');
  if (typeof methodReview.ownerLayer === 'string' && !validOwnerLayers.has(methodReview.ownerLayer)) {
    errors.push(
      `root.methodReview.ownerLayer must be one of: ${Array.from(validOwnerLayers).join(', ')}.`,
    );
  }

  requireNonEmptyString(methodReview, 'ratchetDecision', errors, 'root.methodReview');
  if (
    typeof methodReview.ratchetDecision === 'string'
    && !validRatchetDecisions.has(methodReview.ratchetDecision)
  ) {
    errors.push(
      `root.methodReview.ratchetDecision must be one of: ${Array.from(validRatchetDecisions).join(', ')}.`,
    );
  }
  ...
  requireNonEmptyString(methodReview, 'consumerSpecificLeakage', errors, 'root.methodReview');
  if (
    typeof methodReview.consumerSpecificLeakage === 'string'
    && !validLeakageStates.has(methodReview.consumerSpecificLeakage)
  ) {
    errors.push(
      `root.methodReview.consumerSpecificLeakage must be one of: ${Array.from(validLeakageStates).join(', ')}.`,
    );
  }
}
...
function validateDeliveryGovernanceReview(review, errors) {
  ...
  const validGateStates = new Set(['satisfied', 'gap-recorded', 'not-applicable']);
  const validGithubGateStates = new Set([
    'method-gate',
    'repo-quality-gate',
  ...
  for (const field of ['designGate', 'developmentGate', 'qaAcceptanceGate']) {
    requireNonEmptyString(deliveryGovernanceReview, field, errors, 'root.deliveryGovernanceReview');
    if (
      typeof deliveryGovernanceReview[field] === 'string'
      && !validGateStates.has(deliveryGovernanceReview[field])
    ) {
      errors.push(
        `root.deliveryGovernanceReview.${field} must be one of: ${Array.from(validGateStates).join(', ')}.`,
      );
    }
  }
  ...
  requireNonEmptyString(
    ...
    errors,
    'root.deliveryGovernanceReview',
  );
  if (
    typeof deliveryGovernanceReview.githubGovernanceGate === 'string'
    && !validGithubGateStates.has(deliveryGovernanceReview.githubGovernanceGate)
  ) {
    errors.push(
      `root.deliveryGovernanceReview.githubGovernanceGate must be one of: ${Array.from(validGithubGateStates).join(', ')}.`,
    );
  }
}
