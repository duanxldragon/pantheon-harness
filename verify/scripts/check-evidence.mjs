...
function validateEvidenceFile(filePath, root) {
  ...
  validateSessionEconomics(evidence, result.errors);
...
function validateSessionEconomics(evidence, errors) {
  if (!('sessionEconomics' in evidence)) {
    return;
  }

  if (!isObject(evidence.sessionEconomics)) {
    errors.push('root.sessionEconomics must be an object when present.');
    return;
  }

  const sessionEconomics = evidence.sessionEconomics;
  const validResponseModes = new Set(['terse', 'standard', 'detailed']);
  const validCostSensitivity = new Set(['low', 'medium', 'high']);

  if ('responseMode' in sessionEconomics) {
    if (
      typeof sessionEconomics.responseMode !== 'string'
      || !validResponseModes.has(sessionEconomics.responseMode)
    ) {
      errors.push(
        `root.sessionEconomics.responseMode must be one of: ${Array.from(validResponseModes).join(', ')}.`,
      );
    }
  }

  if ('costSensitivity' in sessionEconomics) {
    if (
      typeof sessionEconomics.costSensitivity !== 'string'
      || !validCostSensitivity.has(sessionEconomics.costSensitivity)
    ) {
      errors.push(
        `root.sessionEconomics.costSensitivity must be one of: ${Array.from(validCostSensitivity).join(', ')}.`,
      );
    }
  }
  ...
  for (const field of [
    ...
    if (field in sessionEconomics && typeof sessionEconomics[field] !== 'number') {
      errors.push(`root.sessionEconomics.${field} must be a number when present.`);
    }
  }

  if ('notes' in sessionEconomics && typeof sessionEconomics.notes !== 'string') {
    errors.push('root.sessionEconomics.notes must be a string when present.');
  }
}
